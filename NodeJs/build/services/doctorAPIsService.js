  ...
  return new Promise(async (resolve, reject) => {
    try {
      let doctors = await db.User.findAll({
        ...
        // Xuất thêm giá trị tại allCode có giá trị tên là (positionData) xuất giá trị valueEn và valueVi
        {
          model: db.Allcode,
          as: 'genderData',
          attributes: ['valueEn', 'valueVi']
        } // Xuất thêm giá trị tại allCode có giá trị tên là (genderData) xuất giá trị valueEn và valueVi
        ],
...
let postCreateInfoDoctor = data => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.id || !data.contentHTML || !data.contentMarkdown) resolve({
        errCode: 1,
...
let putSaveDetailDoctor = data => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.id || !data.contentMarkdown) resolve({
        errCode: 1,
        message: 'Missing parameter!'
      });else {
    ...
    try {
      if (!data.arrSchedule || !data.date || !data.doctorId) {
        ...
        if (schedule && schedule.length > 0) {
          schedule = schedule.map(item => _objectSpread(_objectSpread({}, item), {}, {
...
let postMoreInfoDoctor = data => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.doctorId || !data.priceId || !data.provinceId || !data.paymentId || !data.addressClinic || !data.nameClinic) resolve({
        errCode: 1,
        message: 'Missing parameter!'
      });else {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.doctorId || !data.priceId || !data.provinceId || !data.paymentId || !data.addressClinic || !data.nameClinic) resolve({
        ...
        if (!doctorInfo) resolve({
          errCode: 0,
          message: "Can't find info doctor!"
        });else {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!doctorId) resolve({
        ...
        if (!doctorInfo) doctorInfo = {};else if (doctorInfo && doctorInfo.image) {
...
let postSendRemedy = data => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.fullName || !data.language || !data.date || !data.doctorId || !data.image || !data.patientId || !data.timeType) resolve({
        errCode: 1,
        message: 'Missing parameter!'
      });else {
