...
let buildUrlEmail = (token, doctorId) => {
  let result = "".concat(process.env.URL_REACT, "/verify-booking?token=").concat(token, "&doctorId=").concat(doctorId);
  return result;
};
let postBookAppointment = data => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.language || !data.email || !data.doctorId || !data.timeType || !data.date) resolve({
        errCode: 1,
...
let getListPatientForDoctor = data => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.doctorId || !data.date) resolve({
        errCode: 1,
        message: 'Missing parameter!'
      });else {
        let booking = await db.Booking.findAll({
          ...
          include: [{
            model: db.User,
            as: 'patientData',
            attributes: ['email', 'lastName', 'gender', 'address'],
            include: [{
              model: db.Allcode,
              as: 'genderData',
